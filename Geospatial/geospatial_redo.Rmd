...
```{r Load in the map of the US, warning=FALSE}
...
joined <- left_join(states, raw_aid, by=c("ID" = "NAME"))
...
joined$bin <- bins
bin_labels <- c("$1,120—$1,450", "$1,451—$1,660", "$1,661—$2,020", "$2,021—$2,460", "$2,461—$3,007")
...
(mainland <- ggplot(data = usa) +
    ...
    geom_sf(data = joined, aes(fill = `2018 grant per capita`)) + 
     coord_sf(crs = st_crs(2163), xlim = c(-2500000, 2500000), ylim = c(-2300000, 
         730000))) +
  scale_fill_distiller(palette = "PuBu", direction = 1, labels = bin_labels, name="Per Capita Federal Grant") +
   annotation_custom(
...
column_pc <- ggplot(data = joined, 
  ...
  scale_x_discrete(name= element_blank(), 
                   expand = expand_scale()) +
  scale_fill_brewer(palette = "PuBu", name="Expanded Medicaid?") +
  ...
  labs(title = "Federal Grant Money Supports Health Care Spending") + 
  coord_flip()
column_pc
```
