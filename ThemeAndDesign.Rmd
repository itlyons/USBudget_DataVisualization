...
plot1 <- bar_to_plot + 
    scale_y_continuous( name = "Tax Expenditures in Billions of Dollars", 
...
descr <- labs(title = "The Employer Health Care Deduction Dwarves Other Tax Breaks", 
              subtitle = "Investment & Housing Tax Breaks Make Up 5/10 of the Most Expensive Tax Breaks",
              caption = "Source: United States. Department of Treasury. Tax Policy: Tax Expenditures. \n 
              * These estimates are made relative to current law as of July 1, 2018", 
...
outlay_agency <- outlays %>%
  ...
  summarize(dollars_thousands = sum(dollars_thousands)) 
      ...
      outlay_agency %>% ggplot(aes(x = fiscal_year, y = (dollars_thousands), color = agency_name, group=agency_name)) +
      geom_point() + geom_line() + ggpubr::rotate_x_text() + 
      scale_y_continuous(name = "Outlays (Billions)", labels = scales::dollar_format(scale=.000001, accuracy = 1, suffix = 'B')) +
        theme1 +
        labs(title = "Projected Changes in Selected Departments/Agencies",
          subtitle = "Labor's stimulus funding was short-lived,\n and DoD's expenditures decrerased as wars winded down",
          caption = "Source: United States. Office of Management and Budget. Public Budget Database.",
          x = "Year",
