...
```{r Reshape tax expenditures data, tidy=TRUE}
...
```{r, tidy=TRUE}
time_series <- tax_long %>% select(fiscal_year, Category, expenditure) %>% filter(fiscal_year %in% c(2018, 2023, 2028)) %>% 
  group_by(fiscal_year, Category) %>% tally(expenditure)

time_series <- mutate(.data = time_series, amount_billions = n/1000)
time_series$n <- NULL

bar_to_plot <- ggplot(data=time_series, aes(x=fiscal_year, y=amount_billions, color=Category, fill=Category)) + 
  geom_col(position = "dodge")

plot1 <- bar_to_plot + 
    scale_y_log10(name = "Tax Expenditures in Billions of Nominal Dollars", labels = scales::dollar_format(accuracy = 1, suffix = 'B')) + 
         ...
         * These estimates are made relative to current law as of July 1, 2018", 
         x = "Fiscal Year") +              
          ...
          theme(plot.title = element_text(color="black", size=14, face="bold", hjust = 0.5),
              ...
              axis.title.x = element_text(color="black", size=14), axis.text.x = element_text(angle = 90, hjust = 1, size = 12),
...
```{r Share of Spending, echo=FALSE, tidy=TRUE}
