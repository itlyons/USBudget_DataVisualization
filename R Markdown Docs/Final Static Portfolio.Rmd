---
title: "Static Portfolio: United States Government Revenues and Expenditures"
author: "Ian Lyons"
date: "2/17/2019"
output: 
  html_document: 
...
plot1 <- bar_to_plot + 
    ...
    scale_x_discrete() + 
    labs(title = "The United States Spends Trillions of Dollars\non Tax Breaks Each Year", 
...
Looking at the largest individual tax breaks, it is easy to see the hidden costs of the indirect American approach to health care policy.
...
Passed just after World War 2, the employer health care exemption is one of the cornerstones of the American health care system, with most Americans gaining health insurance through their employer to this day. The estimated cost- over $200 billion a year in foregone revenue- shows the extent to which Americans have enacted a "sunken" welfare state, hidden in the tax code. 
...
details <- c("Employer health care exclusion", 
             "Capital gains tax rate", 
             "Treatment of net imputed rental income", 
             "Defined contribution employer plans", 
             "Employer pension plans", 
             "Accelerated depreciation", 
             "Reduced taxes on income of foreign corporations", 
             "Step-up basis of capital gains", 
             "Charitable contributions", 
             "Capital gains exclusion- home sales")

descr <- labs(title = "The Employer Health Care Deduction Dwarfs Other Tax Breaks", 
              subtitle = "Investor & housing tax breaks are 5 out of the 10 most costly tax expenditures",
              caption = "Source: United States. Department of Treasury. Tax Policy: Tax Expenditures.\nThese estimates are made relative to current law as of July 1, 2018", 
...
expend_plot <- ggplot(data=top10, 
  ...
  coord_flip() +
  scale_y_continuous(name = "Cost of Tax Break", expand = c(0,0),
...
```{r Annual Outlays 1998-2019, message=FALSE, warning=FALSE}
...
facet_data <- mutate(facet_data, 
                     agency_name = ifelse(agency_name == "Department of Defense--Military Programs", "Department of Defense", agency_name))
...
facet_plot <- facet_data %>% ggplot(aes(x = fiscal_year, y = dollars_thousands)) +
  ...
  labs(title = "Defense Generously Funded While Other Departments Languish",
       subtitle = "Red shading indicates recessions, which in the case of 2007 sparked some stimulus spending",
...
ggsave(filename = 'visuals/ShareOfSpending.pdf', width = 12, height = 9)
ShareOfSpending
```
...
themerev <- theme(axis.text.x = element_text(angle = 90, hjust = 0, size = 12),
                  axis.text.y = element_text(size = 12),
...
ggsave(filename = 'visuals/ShareOfRevenue.pdf', width = 11, height = 10)
...
It's informative to note which agencies and/or departments of the federal government are authorized as mandatory spending, which does not have to  be reauthorized every year. On the flip side, some agencies are almost completely discretionary, like the Department of Defense's military . 
...
repel_labs <- geom_label_repel(size=4, 
                               ...
                               force = 5, 
                               box.padding = .7,
                   aes(label = ifelse(((prop_mandatory >= .25 & prop_mandatory <= .80)| agency_name %in% c("Department of Defense--Military Programs", "Department of Health and Human Services", "Department of the Treasury", "Department of Labor", "Department of Transportation", "Department of Homeland Security", "Department of Justice", "National Aeronautics and Space Administration", "National Aeronautics and Space Administration")),
...
discvmand <- ggplot(data = filter(v2_spendtype, mandatory > 0 & discretionary > 0), 
 ...
 expand_limits(x = 1500000000,
               y = 1500000000) + 
  annotate(geom = "text", x = 25000000, y = 1000000, vjust = 0.5, hjust=0, 
           label = "***77% of Treasury's spending goes toward\n interest on the national debt, which is not\nconsidered mandatory or discretionary") + 
  annotate(geom = "segment", 
           x = 100000000, xend = 130000000, 
...
surp_def <- ggplot(data=percent_gdp, aes(x = `Fiscal Year`, y = `Total Revenues`)) +
  ...
  geom_col(aes(x=`Fiscal Year`, 
            ...
            color="black") +
  labs(title = "The United States Has Run Deficits For Most of the Post-WW2 Era",
       subtitle = "More recent deficits are due to tax cuts and a temporary increase in spending and loss of\nrevenues during the Great Recession",
       caption="Source: United States. Office of Management and Budget. Public Budget Database.",
       x = "Fiscal Year",
       y = "Percent of GDP") +
  iltheme + 
...
```{r Federal Aid Data and Some Map Stuff, message=FALSE, warning=FALSE, include=FALSE}
library("ggplot2")
theme_set(theme_bw())
library("sf")
raw_aid <- read_csv("raw_data/state aid - Sheet1.csv")
```
```{r Load in the map of the US, message=FALSE, warning=FALSE, include=FALSE}
...
```
```{r Per Capita Federal Grants, message=FALSE, warning=FALSE}
...
mapped_pc
```
```{r State Bar Chart (Paired with Map), message=FALSE, warning=FALSE}
